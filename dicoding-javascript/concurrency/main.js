...
const fetchingUserFromInternet = (isOffline) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (isOffline) {
        reject(new NetworkError('Gagal mendapatkan dari internet'));
      } else {
...
const gettingUserName = async () => {
  try {
    // Ubah jadi true or false disini
...
(async () => {
  try {
    const name = await gettingUserName();
    console.log(name); // John
  } catch (error) {
    console.log(error); // Gagal mendapatkan data dari internet
  }
})();
/**
